...
public sealed class SubmitHotelierApplicationCommandHandler(IHotelierApplicationRepository applications) : IRequestHandler<SubmitHotelierApplicationCommand, Result>
{
    public async Task<Result> Handle(SubmitHotelierApplicationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await applications.HasPendingForApplicantAsync(request.ApplicantId, cancellationToken))
            return Result.Failure(new Error("HotelierApplication.AlreadyPending", "You already have a pending hotelier application."));
        var latest = await applications.GetLatestByApplicantAsync(request.ApplicantId, cancellationToken);
