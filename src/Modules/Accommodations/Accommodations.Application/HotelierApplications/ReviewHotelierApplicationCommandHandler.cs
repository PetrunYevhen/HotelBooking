...
public sealed class ReviewHotelierApplicationCommandHandler(IHotelierApplicationRepository hotelierApplicationRepository) : IRequestHandler<ReviewHotelierApplicationCommand, Result>
{
    public async Task<Result> Handle(ReviewHotelierApplicationCommand request, CancellationToken cancellationToken)
    {
        var application = await hotelierApplicationRepository.GetByIdAsync(request.ApplicationId, cancellationToken);
        ...
        await hotelierApplicationRepository.UpdateAsync(application, cancellationToken);
