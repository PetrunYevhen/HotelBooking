...
public class DataAccessModule : Module
{
    ...
    protected override void Load(ContainerBuilder builder)
    {
        ...
        builder.Register(c =>
        {
            var dbContextOptionBuilder = new DbContextOptionsBuilder<RoomDbContext>();
            dbContextOptionBuilder.UseNpgsql(_databaseConnectionString);

            dbContextOptionBuilder
                .ReplaceService<IValueConverterSelector, StronglyTypedIdValueConverterSelector>();

            return new RoomDbContext(dbContextOptionBuilder.Options, _loggerFactory);
        })
        .AsSelf()
        .As<DbContext>()
        .InstancePerLifetimeScope();
        
        var infrastructureAssembly = typeof(RoomDbContext).Assembly;
