...
namespace Payments.Infrastructure.Configuration.Processing.Inbox;
    ...
    : IRequestHandler<ProcessInboxCommand>
{
    public async Task Handle(ProcessInboxCommand command, CancellationToken cancellationToken)
    {
        ...
        var ids = await connection.QueryAsync<Guid>(new CommandDefinition(
            """SELECT "Id" FROM "Payments"."InboxMessages" WHERE "ProcessedDate" IS NULL ORDER BY "OccurredOn" LIMIT 100""",
            cancellationToken: cancellationToken));
        foreach (var id in ids)
        {
            await using var messageScope = scope.BeginLifetimeScope();
            var context = messageScope.Resolve<PaymentsDbContext>();
            await using var transaction = await context.Database.BeginTransactionAsync(cancellationToken);
            try
            {
                var messages = await context.Set<InboxMessage>().FromSqlInterpolated(
                    $"""SELECT * FROM "Payments"."InboxMessages" WHERE "Id" = {id} AND "ProcessedDate" IS NULL FOR UPDATE SKIP LOCKED""")
