...
import { createBooking, getBookingQuote, type BookingQuoteDto, type CreateBookingAddOnRequest } from "@/domains/bookings/api/bookings"
import { getHotelAddOns, getHotelById, type HotelAddOnDto, type HotelDetailsDto } from "@/domains/accommodations/api/hotels"
import { getRoomsByHotelId, type RoomDetailsDto } from "@/domains/accommodations/api/rooms"
import { Button } from "@/shared/ui/button"
import { Input } from "@/shared/ui/input"
import { Label } from "@/shared/ui/label"
import { hasAccessToken } from "@/shared/lib/api-client"
import { roomImages } from "@/domains/accommodations/room-images"
import { addDays, calculateNights, toLocalDateValue } from "@/shared/lib/local-date"
import { BookingProgress } from "../_shared/BookingProgress"
import { formatDate, formatMoney } from "../_shared/constants"
...
export function Checkout() {
    ...
    function toggleAddOn(hotelAddOnId: string) { setSelectedAddOnIds((current) => { const next = new Set(current); if (next.has(hotelAddOnId)) next.delete(hotelAddOnId); else next.add(hotelAddOnId); return next }) }
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            if (!hasAccessToken()) { navigate(`/signin?returnTo=${encodeURIComponent(`${location.pathname}${location.search}`)}`); return }
